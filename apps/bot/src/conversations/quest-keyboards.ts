...
export function categoryKeyboard() {
  ...
  for (const category of CATEGORIES) {
    keyboard.text(CATEGORY_LABELS[category], `${QUEST_CATEGORY_CALLBACK_PREFIX}${category}`).row();
  }
  return keyboard;
}
...
export function proofTypeKeyboard() {
  ...
  for (const proofType of PROOF_TYPES) {
    keyboard.text(PROOF_TYPE_LABELS[proofType], `${QUEST_PROOF_CALLBACK_PREFIX}${proofType}`).row();
  }
  return keyboard;
}
